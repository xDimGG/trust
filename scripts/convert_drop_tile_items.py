...
# python.exe .\scripts\convert_drop_tile_items.py .\TerrariaServer_Decompiled\Terraria\WorldGen.cs .\src\world\transpiled\drop_tile_items.rs

import os
import re
import sys
from tree_sitter import TreeCursor
from shared import parser, find_block

raw = open(sys.argv[1], encoding='utf-8-sig').read()

switch_block = find_block(raw, 'switch (tileCache.type)\n      {\n        case 0:')

# remove fluff
switch_block = switch_block.replace('byte.MaxValue', '255').replace('type', 'id')
src = bytes(re.sub(r'\((u?short|u?int)\) |tileCache\.', '', switch_block), 'utf-8')

# get the string value of a node (for some reason doing node.text usually returns None)
def s(node):
...
# cases that can't really be automated because they rely on non-std functions or i was too lazy to do
special_cases = set([
	(3, ),
	(5, 596, 616, 634),
	(52, 62, 382),
...
def walk_switch(cur: TreeCursor, match_var, recursive=False, local_name='', arms_only=False, default_value='0'):
	...
	while cur.goto_next_sibling():
		...
		if cur.node.type == 'default_switch_label':
			...
			nums_t = tuple(sorted(map(int, nums)))
			# if it is strictly increasing
			if len(nums_t) >= 3 and nums_t[-1] - nums_t[0] == len(nums_t) - 1:
				match_arm = f'\n\t{nums_t[0]}..={nums_t[-1]} => '
			else:
				match_arm = f'\n\t{' | '.join(nums)} => '

			if not recursive and nums_t in special_cases:
				cur.goto_parent()
				rust += f'{match_arm}{{\n\t\ttodo!()\n\t}}'
				continue
...
out = """// This file was mostly auto-generated by scripts/%s
// Irregular cases were done by hand

use crate::world::tile::Tile;

impl Tile {
	pub fn get_dropped_item_tile(&self) -> i16 {
		%s
	}
}
""" % (os.path.basename(__file__), walk_switch(cur, 'self.id').replace('\n', '\n\t\t'))
